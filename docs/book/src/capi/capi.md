...
| `ANSIBLE_GALAXY_TOKEN` | API token used for Galaxy authentication (written as a temporary YAML mapping to a token file so it is not exposed via command-line arguments) |
